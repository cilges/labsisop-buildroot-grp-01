...
static int sstf_dispatch(struct request_queue *q, int force){
  ...
  long long int tmp_min, diff;
	...
	if (rq) {
		tmp_min = abs(blk_rq_pos(rq) - last_sector);
		list_for_each_entry(tmp_rq, &nd->queue, queuelist) {
		diff = abs(blk_rq_pos(tmp_rq) - last_sector);
			if (diff < tmp_min) {
				tmp_min = diff;
				rq = tmp_rq;
			}
...
MODULE_AUTHOR("Camila Schiffler, Guilherme Romanini, Joao Feijó");
MODULE_LICENSE("GPL");
